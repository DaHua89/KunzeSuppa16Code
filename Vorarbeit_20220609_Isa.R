...
# 7) YEARS OF EDUCATION: lb0187 (biol dataset; https://paneldata.org/soep-core/data/biol/lb0187)
# 8) WORK DISABILITY:
...
BIOL <- read_dta(file = file.path('biol.dta'), 
               col_select = c('cid','hid', 'syear',   
                              "lb0187" ))  # years of education (7)
...
# i <- dhelp
# j <- "help"
stargazer(data = as.data.frame(i[c(j,"age","yearsedu", "married", "west", "needcare")]), 
